...
export default async function handler(req, res) {
  const { collectionName, filterField, filterValue } = req.body; // Get parameters from request body
  ...
  try {
    ...
    const querySnapshot = await collectionRef
      .where(filterField, "array-contains", filterValue)
      .get();

    // Process the query results
    const documents = [];
    querySnapshot.forEach((doc) => {
      documents.push({ id: doc.id, ...doc.data() }); // Push the document data into the array
    });
