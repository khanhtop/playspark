    ...
    <div className="mx-auto max-w-[1000px] ">
      ...
      <div className=" flex flex-col-reverse lg:flex-row gap-5  shadow-xl shadow-grey border rounded-[10px] my-5 py-10">
        <div className="lg:w-1/2  w-full flex flex-col gap-10 items-center justify-center">
          <h1 className=" font-bold text-2xl px-12 text-start">
            {item.blog_title}
          </h1>
          <div className=" flex flex-col gap-8 text-start text-[16px] px-3 items-start">
            <div className=" flex flex-row gap-1 justify-center items-center">
              {item.blog_text_1 && <SelectImg item={item} num="1" />}
              <p className="text-[22px] text-start">{item.blog_text_1}</p>
            </div>
            <div className=" flex flex-row gap-1 justify-center items-center">
              {item.blog_text_2 && <SelectImg item={item} num="2" />}
              <p className="text-[22px] text-start">{item.blog_text_2}</p>
            </div>
            <div className=" flex flex-row gap-1 justify-center items-center">
              {item.blog_text_3 && <SelectImg item={item} num="3" />}
              <p className="text-[22px] text-start">{item.blog_text_3}</p>
            </div>
          </div>
        </div>
...
const SelectImg = ({ num, item }) => {
  if (num == "1" && item.icon_1) {
    return <img src={item.icon_1.url} />;
  } else if (num == "2" && item.icon_2) {
    return <img src={item.icon_2.url} />;
  } else if (num == "3" && item.icon_3) {
    return <img src={item.icon_3.url} />;
  }
};
