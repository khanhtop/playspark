...
export default class newCricketScene extends Phaser.Scene {
  ...
  create() { 
    ...
    this.add
      ...
      .setInteractive({ cursor: 'pointer' });
    this.add
      .sprite(w / 28 + 10, h / 4.5, 'light')
      .setOrigin(0, 0.5)
      .setDisplaySize(w * 0.04, w * 0.04)
      .setInteractive({ cursor: 'pointer' });
    
    this.lightNumText = this.add.text(w / 28 + w * 0.075 * w / 1248, h / 4.5 , this.scorePanel.light, {...this.country_text_style, 
        fill: '#fff',
