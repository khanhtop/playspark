...
export default class FlyCollectScene extends Phaser.Scene {
  public static instance: FlyCollectScene;
  private params: any;
  ...
  constructor(newGameType: string, newParams: any) {
    ...
    this.params.sponsorLogo = !!this.params.sponsorLogo
      ? this.params.sponsorLogo
      : "/pong/" + gameType + "/fence.png";
  }
  ...
  preload() {
    ...
    this.load.audio(
      "levelup",
      "/pong/" + gameType + "/sound/game-level-complete-trial.mp3"
    );
    this.load.audio(
      "item_light",
      "/pong/" + gameType + "/sound/item-pick-up.mp3"
    );
  ...
  create() {
    ...
    this.additionalSpriteOne = this.add
      .sprite(0, 0, "additionalSpriteOne")
      .setOrigin(0)
      .setDisplaySize(backW, h)
      .setPosition(0, 0);
    this.additionalSpriteOne1 = this.add
      .sprite(0, 0, "additionalSpriteOne")
      .setOrigin(0)
      .setDisplaySize(backW, h)
      .setPosition(this.additionalSpriteOne.x + backW, 0);
    this.additionalSpriteOne2 = this.add
      .sprite(0, 0, "additionalSpriteOne")
      .setOrigin(0)
      .setDisplaySize(backW, h)
      .setPosition(this.additionalSpriteOne.x - backW, 0);
    ...
    // LEVEL HEADER
    this.add
      .sprite(60 - leftOffset, topOffset, "levelBoard")
    ...
    this.add
      .sprite(headerW * 0.5 + 80, topOffset, "header")
      .setOrigin(0.5, 0.5)
      .setScrollFactor(0, 0)
      .setDisplaySize(headerW, 40);
    this.add
      .sprite(90, topOffset - 5, "heartUI")
      .setOrigin(0, 0.5)
    ...
    this.add
      .sprite(headerW * 1.5 + 80, topOffset, "header")
      .setOrigin(0.5, 0.5)
      .setDisplaySize(headerW, 40)
      .setScrollFactor(0, 0);
    this.add
      .sprite(headerW + 90, topOffset - 5, "light")
      .setOrigin(0, 0.5)
      .setDisplaySize(25, 25)
      .setScrollFactor(0, 0);
    ...
    this.leftStatus = {};
    this.leftStatus["magnify"] = this.add
      .sprite(5, topOffset + 50, "magnify")
      .setDisplaySize(30, 30)
      .setOrigin(0, 0.5)
      .setScrollFactor(0, 0)
      .setAlpha(0.3)
      .setVisible(false);
    this.leftStatus["power"] = this.add
      .sprite(5, topOffset + 100, "power")
      .setDisplaySize(30, 30)
      .setOrigin(0, 0.5)
      .setScrollFactor(0, 0)
      .setAlpha(0.3)
      .setVisible(false);
    this.leftStatus["shrink"] = this.add
      .sprite(5, topOffset + 150, "shrink")
      .setDisplaySize(30, 30)
      .setOrigin(0, 0.5)
      .setScrollFactor(0, 0)
      .setAlpha(0.3)
    ...
    const item_x = 60;
    const item_r = 45;
    const shrink = this.add
      .sprite(item_x, h - item_dis, "shrink")
      .setDisplaySize(item_r, item_r)
      .setScrollFactor(0, 0)
      .setInteractive()
      .setDepth(100)
      .setOrigin(1, 0.5);
    ...
    const power = this.add
      .sprite(item_x, h - item_dis * 2, "power")
      .setDisplaySize(item_r, item_r)
      .setScrollFactor(0, 0)
      .setInteractive()
      .setDepth(100)
    ...
    const magnify = this.add
      .sprite(item_x, h - item_dis * 3, "magnify")
      .setDisplaySize(item_r, item_r)
      .setScrollFactor(0, 0)
      .setInteractive()
      .setDepth(100)
    ...
    const heart = this.add
      .sprite(item_x, h - item_dis * 4, "heart")
      .setDisplaySize(item_r, item_r)
      .setScrollFactor(0, 0)
      .setInteractive()
      .setDepth(100)
    ...
    this.popUpTexts["direct_ring"] = this.add
      .text(w / 2, h / 2 - 150, "SWISH X 2", {
        fontFamily: "TitanOne-Regular",
        fontSize: "35px",
        color: "#f08827",
      })
    ...
    this.popUpTexts["level"] = this.add
      .text(w / 2, h / 2 + 100, "LEVEL 2 UNLOCKED", {
        fontFamily: "TitanOne-Regular",
        fontSize: "35px",
        color: "#fed957",
      })
  ...
  initObstacle() {
    ...
    const lb = this.physics.add
      .sprite(
        x + radius * sizeRate * Math.cos((angle * Math.PI) / 180),
        y + radius * sizeRate * Math.sin((angle * Math.PI) / 180),
        "ball"
      )
      .setOrigin(0.5, 0.5)
    ...
    const last = this.physics.add
      .sprite(x + objW / 1.5, y, "ball")
      .setOrigin(0.5, 0.5)
      .setDisplaySize(10, 2 * h)
      .setPushable(false)
      .setVisible(false);

    const last_bottom = this.physics.add
      .sprite(x, h, "ball")
      .setOrigin(1, 0.5)
    ...
    const col_db = this.physics.add.overlap(
      this.ball,
      db,
      () => {
        console.log("HIT TOP");
        if (!HIT_STATUS.DOWN) HIT_STATUS.TOP = true;
      },
      null,
      this
    );
    ...
    // RING SUCCESS....
    const col_tb = this.physics.add.overlap(
      this.ball,
      tb,
      () => {
        ...
        if (!HIT_STATUS.DOWN && HIT_STATUS.TOP) {
          ...
          const addRing =
            (GAME.sequence < 1 ? 1 : GAME.sequence * 2) *
            (STATUS.power ? 2 : 1);
    ...
    // ITEM INIT (percent)
    this.items.push(this.initItem(x, y));
  ...
  initItem(x, y) {
    ...
    const col = this.physics.add.overlap(
      this.ball,
      item,
      () => {
        if (type == "light") {
  ...
  addPopupText(type, val) {
    if (type == "ring") {
      ...
      this.popUpTexts[type].setText(
        `${GAME.sequence == 1 ? "PERFECT!" : "COMBO!"}`
      );
      this.popUpTexts[type].setScale(0);
      this.tweens.add({
        targets: this.popUpTexts[type],
  ...
  startRound() {
    ...
    for (let i = 0; i < 3; i++) {
      this.obstacles.push(this.initObstacle());
    }
  ...
  addItemEffect(type, during = 4000) {
    if (type == "shrink") {
      ...
      this.ball
        .setDisplaySize(ballR * 0.7, ballR * 1.5 * 0.7)
        .setOrigin(0.5, 0.5)
        .setBodySize(ballR * 1.5 * 0.7, ballR * 2.25 * 0.7);
    }
    if (type == "magnify") {
      this.obstacles.forEach((obstacle) => {
        ...
        objArray[2].setPosition(
          x + radius * sizeRate * Math.cos((angle * Math.PI) / 180),
          y + radius * sizeRate * Math.sin((angle * Math.PI) / 180)
        );
        objArray[3].setPosition(
          x - radius * sizeRate * Math.cos((angle * Math.PI) / 180),
          y - radius * sizeRate * Math.sin((angle * Math.PI) / 180)
        );
      });
