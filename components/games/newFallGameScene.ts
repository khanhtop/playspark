...
export default class newFallGameScene extends Phaser.Scene {
  public static instance: newFallGameScene;
  ...
  private staticHeart: any;
  private staticText: any;
  load: any;
  game: any;
  sound: any;
  physics: any;
  ...
  constructor(newGameType: string, newParams: any) {
    super();
    newFallGameScene.instance = this;
  ...
  preload() {
    ...
    this.load.image("ball", getImageWithSize(this.params.objectSprite, 300, 300));
    this.load.image("bg", getImageWithSize(this.params.backgroundSprite, 1200));
  ...
  create() {
    ...
    // End Player Anim
    this.head = this.physics.add.sprite(mW, h - goalH - playerH / 2, "head")
    ...
    this.staticHeart = this.add.sprite(mW - 60, mH + offsetYY, 'heart')
      .setDisplaySize(60, 60).setScale(0, 0);
    this.staticText = this.add.text(mW + 30, mH + offsetYY, "+ 1", {
        fontFamily: "TitanOne-Regular",
        fontSize: "45px",
        color: "#ffffff",
        stroke: "#345e8e",
      }).setOrigin(0.5, 0.5).setScale(0, 0)
    this.staticBonusScreen.add(this.staticHeart)
    this.staticBonusScreen.add(this.staticText)
  ...
  update(time, delta) {
    ...
    this.head.setPosition(this.player.x, this.player.y)
