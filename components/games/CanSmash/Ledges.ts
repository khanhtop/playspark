...
export class Ledges {
  ...
  init(pos: Vector3, type: PlatformMovmentTypes, range: number = 0, speed: number = 0) {
    ...
    this.type = type;
    this.range = range;
    let viewer = new PhysicsViewer();
    if (this.canContainer == null) {
      this.canContainer = MeshBuilder.CreateBox("box", {
        width: 0.5,
        height: 0.05,
        depth: 0.25,
      });
      ...
      const result = cloneMesh.get(Meshs.data.ledges);
      ...
      this.canBody = this.meshs[0];
      this.canBody.name = "body";

     // this.meshs[2].material = Materials.instance.barrelUp;
      this.meshs[1].material = Materials.instance.ledgesBody;
