...
export default function MarketPlace({}) {
    ...
    return (
        <div className="flex">
            {games.map((item, key) => (
                <GameCard buttonText="Add To My Games" game={item} added={context.profile?.myGames?.includes(games.findIndex(a => a.id === item.id))} onDemo={() => setDemo(item.id)} onAdd={() => addToLibrary(games.findIndex(a => a.id === item.id))}/>
            ))}
            {demo &&
